...
- Скачивание из трёх разделов: docs, projectdocs, printformdocs
...
```env
SITE_BASE_URL=https://test.xxx.xxx
SITE_LOGIN_ENDPOINT=/api/auth/login
SITE_LOGIN=your_login
SITE_PASSWORD=your_password
# Или готовый токен (План Б)
SITE_TOKEN=your_token_if_available
SITE_VERIFY_SSL=false
```
...
### 2. Авторизация
- **План А**: Автоматическая авторизация через логин/пароль
- **План Б**: Использование готового токена из `.env`
- При истечении токена — автоматический перелогин

### 3. Скачивание файлов
Для каждого договора:
1. GET запросы к трём разделам API:
   - `/api/v1/document/attachData/{contract}/docs/`
   - `/api/v1/document/attachData/{contract}/projectdocs/`
   - `/api/v1/document/attachData/{contract}/printformdocs/`
2. Для каждого файла из списка:
   - GET запрос к `/api/v1/attachment/download/{documentKey}/{key}`
   - Сохранение в `./downloaded_files/{contract_number}/{filename}`
3. Проверка существующих файлов (пропуск при совпадении размера)
4. Немедленная запись метаданных в БД
...
### Файлы не скачиваются (401/403)
1. Проверьте логин/пароль в `.env`
2. Попробуйте План Б — вручную получите токен из браузера:
   ```env
   SITE_TOKEN=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
   ```

### Много пустых файлов логов
Используется единый файл `app.log`. Старые файлы можно удалить:
```bash
rm logs/app_*.log  # оставить только app.log
```

## Производительность

### Рекомендации
- Для SSD: `MAX_WORKERS=4`, `MAX_OCR_WORKERS=2`
- Для HDD: уменьшите до `2` и `1` соответственно
- При большом количестве OCR: увеличьте `MAX_OCR_WORKERS` до 4 (требует больше CPU)
- Для сети с задержками: увеличьте `DOWNLOAD_RETRY_COUNT` до 5

### Оптимизация
- Пропуск существующих файлов (`SKIP_IF_EXISTS=true`)
- Пакетная запись в БД (`BATCH_SIZE=50`)
- Частичные хэши для быстрой дедупликации
